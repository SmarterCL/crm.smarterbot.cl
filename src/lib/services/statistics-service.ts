...
export async function getStatistics(metricName: string, days = 7): Promise<Statistic[]> {
  ...
  try {
    ...
    if (error) {
      ...
      // Log other DB errors but don't fail
      console.warn(`Database error fetching statistics for ${metricName}:`, error.message)
      return []
    }

    return data || []
  } catch (err) {
    // Silence fetch/connection errors during build - "dejar pasar"
    return []
  }
}
...
export async function getDailyStatistics(): Promise<any> {
  ...
  try {
    ...
    // Silence fetch/connection errors during build
    return {}
  }
}
